...
\033[95m█████  \033[93m ███████ ███████\033[95m █████ \033[94m██      \033[92m██     \033[91m ██ 
\033[95m██  ██ \033[93m ██   ██ ██   ██\033[95m       \033[94m██      \033[92m██     \033[91m ██ 
\033[95m██   ██\033[93m ██   ██ ██   ██\033[95m       \033[94m ██████ \033[92m███████\033[91m ██ v1.0.5
\033[0m                                          
"""
...
watch_history = {'anime' : {}, 'last' : {}, 'airing' : {}}
try:
    with open('./history.txt','r',encoding='utf-8') as histo: #membaca config histo jika ada file history.txt
        watch_history = eval(histo.read())
except:
    with open('./history.txt','w',encoding='utf-8') as histo:  #menulis config histo jika tidak ada history.txt
        histo.write(str(watch_history))

while True:
    # if dcrpc:
    #     RPC.update(state="In Main Menu", details="Browsing anime") #update dict
    print("\033[93mtype H for history\ntype R to resume watching\ntype A to see rencently uploaded\n\033[1mor just type the anime title to search\033\n")
    ...
    if query in ('H','h'):
        ...
        for i in range(len(animes_k)): #menampilkan history
            if i%2 == 0:
